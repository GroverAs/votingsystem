...
public interface DishRepository extends BaseRepository<Dish> {
    ...
    @Query("SELECT d FROM Dish d WHERE d.restaurant.id=?1 ORDER BY d.actualDate DESC")
    List<Dish> getAllDishesByRestaurantId(int restaurantId);

    @Query("SELECT d FROM Dish d WHERE d.restaurant.id=:restaurantId AND d.actualDate=:actualDate")
    List<Dish> getAllByDate(int restaurantId, LocalDate actualDate);

    default Dish checkRelationToRestaurant(int id, int restaurantId) {
