...
public interface RestaurantRepository extends BaseRepository<Restaurant> {
    @Query("SELECT r FROM Restaurant r JOIN FETCH r.dishes d WHERE d.actualDate=:date AND r.id=:id")
    Optional<Restaurant> getByIdAndLocalDate(int id, LocalDate date);

    @Query("SELECT DISTINCT r from Restaurant r JOIN FETCH r.dishes d WHERE d.actualDate=:date")
